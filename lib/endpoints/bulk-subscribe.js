    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (operation === "batchAdd") {
                ...
                yield req.db.query("INSERT INTO currently_subscribed (topic_id, firebase_id) SELECT $1, * FROM UNNEST ($2::text[]) ON CONFLICT DO NOTHING", [req.params.topic_name, successfulIDs]);
            }
            else {
                yield req.db.query(`DELETE FROM currently_subscribed WHERE topic_id = $1 AND firebase_id IN (SELECT * FROM UNNEST ($2::text[]))`, [req.params.topic_name, successfulIDs]);
            }
