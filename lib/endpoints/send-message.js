...
function sendMessage(target, msgType, body, log, parseIOSFromPayload) {
    ...
    if (errors.length > 0) {
        log.error({ errors: errors }, "Failed to send message");
...
const parseTopicResponse = function (res, log) {
    return res.text()
        ...
        return Promise.resolve()
            ...
            .catch((err) => {
            log.error({ text: text }, "Could not parse JSON response");
...
exports.sendMessageToTopic = function (req, res, next) {
    Promise.resolve()
        ...
        let topics = topicSelector
            .split("+")
            .map(namespace_topic_1.namespaceTopic);
        req.log.info({
            target: topics,
            action: 'send',
            sendType: 'topic'
        }, "Received request to send message.");
        return sendMessage({ topics: topics }, MessageSendType.Topic, req.body, req.log, parseIOSFromPayload);
    })
        .then((finalId) => {
        res.json({
            id: finalId
...
exports.sendMessageToRegistration = function (req, res, next) {
    Promise.resolve()
        ...
        return sendMessage({ registration: registration }, MessageSendType.Registration, req.body, req.log, parseIOSFromPayload);
    })
        .then((finalId) => {
        res.json({
            id: finalId
